...
;(function ($, window, document) {
	...
	var _templates = {
		...
		image: '<form id="imageForm" enctype="multipart/form-data" method="POST">' +
        '<div><label for="link">{url}</label> <input type="text" id="image" value="http://" /></div>' +
        '<div><label for="upload">{upload}</label> <input type="file" id="image_up" value="" /></div>' +
        '<div><hr /></div>' +
				...
				'<div><input type="button" class="button" value="{insert}" /></div></form>',
...
// TODO: Make configurable
	...
	$.sceditor.commands = {
		...
		image: {
            ...
            val_up = content.find('#image_up').val(),
          ...
          //File Upload
					if (val_up) {
					    ...
					    $.ajax({
					        ...
					        success: function (data) {
					            insertImage(e, editor, width, height, file.name, true, data);
					        },
					        error: function () {
					            alert(editor._('An error occured when uploading the file. Only images can be uploaded (jpg, png, gif, bmp, svg)'));
					        },
					        data: formData,
					        cache: false,
					        contentType: false,
                  processData: false
					    ...
					    if (val.length < 9) {
					        alert(editor._('No URL given'));
					    } else {
					        insertImage(e, editor, width, height, val, false);
					    }
					}
				});

				function insertImage(e, editor, width, height, fileName, wasUploaded, data) {
				    var serializedData = {};
				    if (data) {
				        serializedData = eval(data)[0];
				    }
				    if (serializedData.status == "error") {
				        alert(editor._('An error occured when uploading the file. Only images can be uploaded (jpg, png, gif, bmp, svg)'));
				    } else {
				        var attrs = '';
				        if (width)
				        ...
				        editor.wysiwygEditorInsertHtml('<img' + attrs + ' src="' + fileName + '" />');
